...
function SpriteAvatar({ charClass, charSex, size = 56 }: { charClass: number; charSex: string; size?: number }) {
  const job = getJob(charClass);
  const [imgFailed, setImgFailed] = useState(false);
  const spriteUrl = `https://static.divine-pride.net/images/jobs/png/${charClass}.png`;

  if (!imgFailed) {
    return (
      <div style={{ width: size, height: size, borderRadius: '50%', overflow: 'hidden', background: job.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        <img
          src={spriteUrl}
          alt={job.label}
          width={size}
...
export default function AvatarPicker({
  ...
  function handleSelect(char: Char) {
    ...
    startTransition(async () => {
      await selectAvatarAction(char.name, char.class, char.sex);
      setOpen(false);
    });
  }
  ...
  return (
    <>
      ...
      >
        {selectedChar ? (
          <SpriteAvatar charClass={selectedChar.class} charSex={selectedChar.sex} size={72} />
        ) : (
        ...
        <div className={styles.modalBackdrop} onClick={() => setOpen(false)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <p className={styles.modalSubtitle}>
              Selecciona el personaje cuyo sprite usarás como foto de perfil.
            </p>
                ...
                {chars.map(char => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isPending}
                    >
                      <SpriteAvatar charClass={char.class} charSex={char.sex} size={52} />
                      <div className={styles.charPickerInfo}>
                        <span className={styles.charPickerName}>{char.name}</span>
                        <span className={styles.charPickerJob} style={{ color: job.color }}>
                          {job.label}
                        </span>
                        <span className={styles.charPickerLv}>Lv {char.base_level}</span>
                      </div>
